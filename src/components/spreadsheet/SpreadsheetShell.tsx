...
const NAV: NavLeaf[] = [
  { label: "Sheet", href: "/spreadsheet", icon: <Table2 className="w-5 h-5" /> },
]

function isActivePath(pathname: string, href: string) {
  if (href === "/spreadsheet") return pathname === "/spreadsheet"
  return pathname === href || pathname.startsWith(href + "/")
}

export const SpreadsheetShell: React.FC<{ userName: string; children: React.ReactNode }> = ({ userName, children }) => {
  const pathname = usePathname() || "/spreadsheet"
  const [menuOpen, setMenuOpen] = useState(false)
  const initial = userName.trim().charAt(0).toUpperCase() || "?"
  const activeLabel = NAV.find((item) => isActivePath(pathname, item.href))?.label ?? "Spreadsheet"
      ...
      <aside className="hidden lg:flex fixed inset-y-0 left-0 z-40 w-60 flex-col justify-between bg-gradient-to-b from-[#0a2540] via-[#09356b] to-[#041c38] text-white shadow-2xl border-r border-blue-900/40">
        <div>
          <div className="h-20 px-5 flex flex-col justify-center border-b border-blue-800/40">
            ...
            <span className="text-[10px] text-blue-200 font-semibold tracking-tight mt-0.5">From Spreadsheet</span>
          </div>
