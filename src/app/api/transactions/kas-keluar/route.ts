...
export async function POST(request: Request) {
  ...
  // Owner+Direktur+Admin — Kas Keluar memang termasuk tugas admin (satu-satunya bagian
  // Keuangan yang kebuka buat dia, lihat navItemsForRole di Sidebar.tsx). Role lain
  // (mis. sysadmin Monitoring) tidak boleh.
  if (user.role !== "owner" && user.role !== "direktur" && user.role !== "admin") {
    return NextResponse.json({ error: "Tidak punya akses input Kas Keluar" }, { status: 403 })
  }
