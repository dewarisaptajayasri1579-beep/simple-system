...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await prisma.leadLostReason.update({ where: { id }, data })
  await logAudit({ actorUserId: user.id, action: "marketing.lostreason.update", entityType: "lead_lost_reason", entityId: id, after: data })
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params
  const used = await prisma.lead.count({ where: { lostReasonId: id } })
  ...
  await logAudit({ actorUserId: user.id, action: "marketing.lostreason.delete", entityType: "lead_lost_reason", entityId: id })
  return NextResponse.json({ ok: true })
}
