...
export async function GET() {
  ...
  const [segments, sources, lostReasons, disqualifyReasons, users, activityTypes, followUpResultTypes, buyingPowerTiers] = await Promise.all([
    ...
    prisma.leadDisqualifyReason.findMany({
      where: { isActive: true },
  ...
  return NextResponse.json({ segments, sources, lostReasons, disqualifyReasons, users, activityTypes, followUpResultTypes, buyingPowerTiers })
}
