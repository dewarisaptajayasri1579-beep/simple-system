...
- **Disk per aplikasi** (`Application`-nya sendiri, bukan cuma agregat VPS): dari command yang sama di `getVpsDockerDiskUsage()`, jalankan juga `docker ps -a -s --format '{{json .}}'` (fungsi `parseDockerPsLine()`). Coolify nandain tiap container aplikasi dengan label `coolify.name=<uuid>` yang persis sama dengan `Application.coolifyUuid` kita — dicocokkan di `GET /api/monitoring/vps` (baca dari cache `appDiskUsage`, bukan live), hasilnya field `diskUsage: { size, virtualSize }` per aplikasi di response. `size` itu writable layer container itu sendiri (biasanya kecil buat app stateless), `virtualSize` itu total image+layer — **cuma perkiraan kasar** karena base layer bisa dipakai bareng beberapa aplikasi sekaligus (bukan porsi eksklusif dari total disk VPS). Container database (`coolify.databaseId`, bukan `coolify.name`) tidak ke-match, jadi tidak ketuker.
  - Ditampilkan sebagai kolom "Disk" di tabel aplikasi (lihat bagian 6.4 buat layout tabelnya). Kosong (`-`) kalau aplikasi belum pernah sync dari Coolify (tidak punya `coolifyUuid`), container-nya tidak ketemu (mis. sedang tidak jalan), atau cache disk Docker VPS itu belum pernah di-"Cek Sekarang".

### 6.4 Aplikasi — git repo & domain (auto-sync dari Coolify API)

- **Layout tabel** (`src/components/monitoring/VpsMonitoring.tsx`) — kolom digabung biar tidak kebanyakan kolom sempit: **Aplikasi** (nama + domain), **Git / Database** (git repo + info database), **Diakses / Backup** (terakhir diakses+siapa, terakhir backup+lokasi), **Disk**, **Domain Habis**, **Aksi** (owner-only).
