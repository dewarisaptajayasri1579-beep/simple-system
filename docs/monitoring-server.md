...
- **Tombol "Bersihkan yang Tidak Terpakai"** (owner-only, ada konfirmasi dulu) — `POST /api/monitoring/vps/[id]/prune-unused`, jalankan 2 command lewat SSH+sudo: `docker system prune -af` (image lama/dangling, container berhenti, network nganggur) **DAN** `docker buildx prune -af` (cache BuildKit — beda dari system prune, cache builder ini tersimpan di volume sendiri seperti `buildx_buildkit_*` yang TIDAK ke-cover oleh `docker system prune` walau sama-sama kategori "volume" di Docker). **SENGAJA TIDAK PERNAH pakai flag `--volumes`** di command mana pun — volume data aplikasi/database (lihat bagian 6.3) tidak pernah tersentuh sama sekali, apa pun kondisinya. Setelah berhasil, otomatis trigger `getVpsDockerDiskUsage()` ulang biar angkanya langsung ke-refresh.
  - Coolify sendiri sebenarnya sudah punya auto-cleanup bawaan juga (lihat setting `docker_cleanup_threshold`/`docker_cleanup_frequency` di server settings-nya, defaultnya cuma jalan kalau disk usage > 80%) — tombol ini buat yang mau bersih-bersih manual sebelum itu.
