...
- Ditampilkan di UI (`src/components/monitoring/VpsMonitoring.tsx`, `DOCKER_TYPE_META`) sebagai list 4 baris progress bar (bukan kartu — kartu disisakan buat ringkasan jumlah Aplikasi/Database) di bawah bar Disk Space tiap kartu VPS. Tiap kategori punya warna beda (Images amber, Containers biru, Local Volumes hijau, Build Cache ungu) dan 1 baris deskripsi singkat di bawahnya menjelaskan artinya buat yang belum familiar istilah Docker-nya. Lebar bar proporsional ke ukuran relatif tiap kategori (bukan ke reclaimable) — info reclaimable ("sampah") ditulis di teks sebelah kanan tiap baris.
- Bar **Disk Space utama** (yang nunjukkin `df`) juga di-highlight bertingkat 3 warna berdasarkan data breakdown ini: biru = porsi Local Volumes, oranye = porsi Images yang reclaimable, abu-abu = sisa terpakai lainnya (OS, image aktif, container). Fallback ke bar 1 warna polos kalau `dockerDisk` null (gagal sudo/docker).
...
- **Disk per aplikasi** (`Application`-nya sendiri, bukan cuma agregat VPS): dari command yang sama (digabung 1 SSH connection), jalankan juga `docker ps -a -s --format '{{json .}}'` (fungsi `parseDockerPsLine()`). Coolify nandain tiap container aplikasi dengan label `coolify.name=<uuid>` yang persis sama dengan `Application.coolifyUuid` kita — dicocokkan di `GET /api/monitoring/vps` (bukan di lib SSH-nya), hasilnya field `diskUsage: { size, virtualSize }` per aplikasi di response. `size` itu writable layer container itu sendiri (biasanya kecil buat app stateless), `virtualSize` itu total image+layer — **cuma perkiraan kasar** karena base layer bisa dipakai bareng beberapa aplikasi sekaligus (bukan porsi eksklusif dari total disk VPS). Container database (`coolify.databaseId`, bukan `coolify.name`) tidak ke-match, jadi tidak ketuker.
  - Ditampilkan sebagai baris "Disk" di kartu tiap aplikasi. Kosong (`-`) kalau aplikasi belum pernah sync dari Coolify (tidak punya `coolifyUuid`) atau container-nya tidak ketemu (mis. sedang tidak jalan).
  - Skrip gabungan (df + backup + `docker system df` + `docker ps -s`) di VPS Dewari butuh **~24 detik total**, timeout digeser jadi 40 detik (lihat `getVpsDiskAndBackup()` di `src/lib/monitoring/ssh.ts`).
